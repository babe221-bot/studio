...
test.describe('Critical User Journey: Select -> Configure -> Checkout', () => {
  ...
  test('Should complete the full flow from selection to PDF download', async ({
    page,
  }) => {
    ...
    // Verify total cost is visible before adding
    const totalCostLabel = page.getByText('Ukupni trošak');
    await expect(totalCostLabel).toBeVisible();

    const addButton = page.getByRole('button', {
      name: 'Dodaj stavku u nalog',
    });
    await addButton.click();

    // Verify item added to order list
    const orderItem = page.locator(
      'div.flex.items-center.justify-between.rounded-lg.border'
    );
