...
const FinishModal: React.FC<FinishModalProps> = ({
  ...
  const handleSave = () => {
    ...
    if (Object.keys(newErrors).length > 0) {
      ...
      document
        .getElementById('finish-form-errors')
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <FocusScope asChild loop trapped>
        ...
        >
          <DialogHeader>
            <DialogTitle id="finish-modal-title">
              {item ? 'Uredi obradu' : 'Nova obrada lica'}
            </DialogTitle>
            <DialogDescription id="finish-modal-desc" className="sr-only">
              Forma za {item ? 'uređivanje' : 'dodavanje'} obrade lica. Naziv je
              obavezan.
            </DialogDescription>
          </DialogHeader>
