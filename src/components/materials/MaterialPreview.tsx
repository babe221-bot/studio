...
function StoneSlab({ material }: { material: Material }) {
  const { textures, isLoading } = useMaterialTextures(material.id);
  const meshRef = useRef<THREE.Mesh>(null);

  // Default values if textures are loading
  const roughness = material.roughness ?? 0.15;
  const clearcoat = material.clearcoat ?? 0.2;
