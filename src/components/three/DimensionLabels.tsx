...
export const DimensionLabels: React.FC<DimensionLabelsProps> = ({
  dims,
  visible,
}) => {
  const [labelTextures, setLabelTextures] = React.useState<
    {
  ...
  useEffect(() => {
    ...
    const newLabels = [
      {
        text: `${length} cm`,
        position: [0, h + 0.35, vizW / 2 + 0.35] as [number, number, number],
      },
      {
        text: `${width} cm`,
        position: [-vizL / 2 - 0.35, h + 0.35, 0] as [number, number, number],
      },
      {
        text: `${height} cm`,
        position: [vizL / 2 + 0.35, h / 2, -vizW / 2 - 0.35] as [
          number,
          number,
          number,
        ],
