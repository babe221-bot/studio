...
interface TemplateManagerProps {
  ...
  onSave: (name: string, items: OrderItem[]) => Promise<any>;
...
export function TemplateManager({
  ...
  const handleSave = async () => {
    if (!templateName.trim()) {
      toast({
        title: 'Greška',
        description: 'Unesite naziv predloška.',
        variant: 'destructive',
      });
      return;
    }
    if (currentItems.length === 0) {
      toast({
        title: 'Greška',
        description:
          'Nalog je prazan. Dodajte stavke prije spremanja predloška.',
        variant: 'destructive',
      });
      return;
    }
    ...
    setIsSaving(true);
    try {
      await onSave(templateName, currentItems);
      setTemplateName('');
      setIsDialogOpen(false);
      toast({
        title: 'Predložak spremljen',
        description: `Predložak "${templateName}" je uspješno kreiran.`,
      });
    } catch (err: any) {
      toast({
        title: 'Greška',
        description: err.message || 'Neuspjelo spremanje predloška.',
        variant: 'destructive',
      });
    } finally {
      setIsSaving(false);
          ...
          <div className="space-y-4 py-4">
            <div className="space-y-2">
              <Label htmlFor="template-name">
                Naziv predloška (npr. Standardni kuhinjski otok)
              </Label>
              <Input
                id="template-name"
                value={templateName}
            ...
            <p className="text-sm text-muted-foreground">
              Ovo će spremiti trenutnih {currentItems.length} stavki iz naloga
              kao višekratno upotrebljiv predložak.
            </p>
          </div>
          <DialogFooter>
                    ...
                    <div
                      ...
                      onClick={() => {
                        ...
                        toast({
                          ...
                          description: `Projekt "${template.name}" je učitan.`,
                        });
                      }}
                    >
