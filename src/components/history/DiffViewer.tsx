          ...
          <TableBody>
            {diffs.map(({ type, item, itemA, itemB, id }) => (
              ...
              >
                ...
                <TableCell>
                  {type === 'added' && (
                    <Badge className="bg-green-600">Dodano</Badge>
                  )}
                  {type === 'removed' && (
                    <Badge variant="destructive">Uklonjeno</Badge>
                  )}
                  {type === 'modified' && (
                    <Badge
                      variant="secondary"
                  ...
                  {type === 'unchanged' && (
                    <Badge variant="outline">Nepromijenjeno</Badge>
                  )}
                </TableCell>
                <TableCell className="text-xs">
                  {type === 'modified' && itemA && itemB && (
                    <div className="space-y-1">
                      {itemA.dims.length !== itemB.dims.length && (
                        <div>
                          Dužina: {itemA.dims.length} → {itemB.dims.length}
                        </div>
                      )}
                      {itemA.dims.width !== itemB.dims.width && (
                        <div>
                          Širina: {itemA.dims.width} → {itemB.dims.width}
                        </div>
                      )}
                      {itemA.material.id !== itemB.material.id && (
                        <div>
                          Materijal: {itemA.material.name} →{' '}
                          {itemB.material.name}
                        </div>
                      )}
                      {itemA.finish.id !== itemB.finish.id && (
                        <div>
                          Obrada: {itemA.finish.name} → {itemB.finish.name}
                        </div>
                      )}
                      {itemA.totalCost !== itemB.totalCost && (
                        <div>
                          Cijena: €{itemA.totalCost.toFixed(2)} → €
                          {itemB.totalCost.toFixed(2)}
                        </div>
                      )}
                    </div>
                  )}
