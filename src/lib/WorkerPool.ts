...
export class WorkerPool<TInput = unknown, TOutput = unknown> {
  ...
  execute(input: TInput, config: JobConfig = {}): Promise<TOutput> {
    ...
    return new Promise((resolve, reject) => {
      ...
      console.debug(
        `[WorkerPool] Job ${job.id} submitted, queue length: ${this.jobQueue.length}`
      );
      this.log(
        `[JOB QUEUED] Job ${job.id} queued, queue length: ${this.jobQueue.length}`
      );

      // Log queue warning at 80% capacity
      if (this.jobQueue.length >= this.config.maxQueueSize * 0.8) {
        this.log(
          `[QUEUE WARNING] Queue at ${Math.round((this.jobQueue.length / this.config.maxQueueSize) * 100)}% capacity`
        );
      }
  ...
  executeBatch(inputs: TInput[], config: JobConfig = {}): Promise<TOutput[]> {
    return Promise.all(inputs.map((input) => this.execute(input, config)));
  }
  ...
  getStats(): PoolStats {
    ...
    if (
      this.config.debug &&
      this.stats.totalProcessed % 10 === 0 &&
  ...
  private executeJob(
    ...
    state.worker.addEventListener('message', state.messageHandler, {
      once: true,
    });
  ...
  private handleJobTimeout(
    ...
    console.error(
      `[WorkerPool] Job ${job.id} failed:`,
      new Error(`Job timed out after ${job.timeout}ms`)
    );
    this.log(
      `[WORKER TIMEOUT] Job ${job.id} timed out after ${job.timeout}ms on worker ${state.id}`
    );
  ...
  private handleWorkerError(
    ...
    const failureRate =
      totalOps > 0 ? (this.stats.totalFailed / totalOps) * 100 : 0;
    ...
    if (failureRate > 10) {
      this.log(
        `[HIGH FAILURE RATE] ${failureRate.toFixed(1)}% of operations have failed`
      );
    }
  ...
  private checkWorkerHealth(): void {
    ...
    if (currentCount < targetCount) {
      this.log(
        `Worker count mismatch: ${currentCount}/${targetCount}, creating missing workers`
      );
...
let geometryWorkerPool: WorkerPool<GeometryJobInput, GeometryJobOutput> | null =
  null;

export function getGeometryWorkerPool(): WorkerPool<
  GeometryJobInput,
  GeometryJobOutput
> {
