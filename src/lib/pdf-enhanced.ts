...
async function generateProfileDiagram(
  profile: EdgeProfile,
  edgeName: string
): Promise<string> {
...
export async function generateEnhancedPdf(
  ...
  try {
    ...
    doc.setFont('Helvetica', 'bold');
    doc.text(
      customerName || 'Veleprodajni kupac',
      customerBoxX + 5,
      customerBoxY + 18
    );
    doc.setFontSize(9);
    ...
    for (let i = 0; i < orderItems.length; i++) {
      ...
      doc.setFontSize(11);
      doc.text(
        `${i + 1}. ${item.id} - ${item.material.name}`,
        margin + 2,
        cursorY + 2
      );
      cursorY += 12;
      ...
      if (images3D[i]) {
        try {
          // Only add image if it's a valid data URI
          if (images3D[i]!.length > 100) {
            let imgData = images3D[i]!;
            if (imgData.startsWith('data:image/svg+xml')) {
              imgData = await svgToPng(imgData, 600, 450);
            }
            doc.addImage(
              imgData,
              'PNG',
      ...
      try {
        const topView = await generateTechnicalDrawing2D(
          item,
          edgeNames,
          'top'
        );
        ...
        if (!isMobile || (isMobile && i === 0)) {
          const sideView = await generateTechnicalDrawing2D(
            item,
            edgeNames,
            'side'
          );
        ...
        console.error('2D Drawing failed', e);
      }
      ...
      const itemTableData = [
        [
          'Materijal',
          `${item.material.name} (${item.dims.height}cm)`,
          `€${costs.materialCost.toFixed(2)}`,
        ],
        ['Obrada lica', item.finish.name, `€${costs.finishCost.toFixed(2)}`],
        [
          'Obrada ivica',
          `${item.profile.name} (${edgeStr})`,
          `€${costs.edgeCost.toFixed(2)}`,
        ],
      ];
      if (costs.okapnikCost > 0)
        itemTableData.push([
          'Okapnik',
          'Izrada okapnika',
          `€${costs.okapnikCost.toFixed(2)}`,
        ]);
      itemTableData.push([
        {
          content: 'SUBTOTAL',
        ...
        {
          content: `€${item.totalCost.toFixed(2)}`,
    ...
    if (notes) {
      ...
      doc.setFontSize(10);
      doc.setFont('Helvetica', 'bold');
      doc.text('DODATNE NAPOMENE:', margin, cursorY);
      cursorY += 6;
      doc.setFont('Helvetica', 'normal');
      doc.setFontSize(9);
    ...
    doc.setFont('Helvetica', 'bold');
    doc.text(
      `€ ${totalCost.toLocaleString('hr-HR', { minimumFractionDigits: 2 })}`,
      pageWidth - margin - 5,
      cursorY + 25,
      { align: 'right' }
    );
    ...
    doc.text(
      `Ukupna masa: ${totalWeight.toFixed(1)} kg`,
      summaryX + 5,
      cursorY + 35
    );
    doc.text(
      `PDV (25%) uključen: € ${(totalCost * 0.2).toFixed(2)}`,
      summaryX + 5,
      cursorY + 40
    );
    ...
    doc.setFontSize(8);
    doc.text(
      'Napomena: Ova ponuda je informativnog karaktera. Rok izrade počinje teći od uplate avansa.',
      margin,
      cursorY
    );
    cursorY += 4;
    doc.text(
      'Uvjeti plaćanja: 50% avans, 50% prije isporuke. Svi proizvodi ostaju u vlasništvu prodavatelja do potpune isplate.',
      margin,
      cursorY
    );
