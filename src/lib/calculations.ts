...
export function calculateOrderCosts({
  ...
  const weight_kg_piece =
    (length * width * height * selectedMaterial.density) / 1000;
  ...
  const edgeProcessingCost_piece =
    processed_perimeter_m * selectedProfile.cost_m;
  ...
  const surfaceProcessingCost_piece =
    surfaceArea_m2_piece * selectedFinish.cost_sqm;
  const processingCost_piece =
    surfaceProcessingCost_piece + edgeProcessingCost_piece;
  const totalCost_piece =
    materialCost_piece + processingCost_piece + okapnikCost_piece;
