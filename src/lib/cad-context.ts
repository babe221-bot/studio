...
function formatOrderItem(item: OrderItem, index: number): string {
  ...
  const unit =
    item.orderUnit === 'sqm' ? 'm²' : item.orderUnit === 'lm' ? 'fm' : 'kom';
 ...
 */
function formatProcessedEdges(edges: {
  front: boolean;
  back: boolean;
  left: boolean;
  right: boolean;
...
export function buildCADContext(data: CADContextData): string {
  ...
  if (data.selectedMaterial) {
    parts.push(
      `Odabrani materijal: ${data.selectedMaterial.name} (gustoća: ${data.selectedMaterial.density} kg/m³, cijena: ${data.selectedMaterial.cost_sqm} €/m²)`
    );
  }
  ...
  if (data.selectedFinish) {
    parts.push(
      `Odabrana površinska obrada: ${data.selectedFinish.name} (${data.selectedFinish.cost_sqm} €/m²)`
    );
  }
  ...
  if (data.selectedProfile) {
    parts.push(
      `Odabrani rubni profil: ${data.selectedProfile.name} (${data.selectedProfile.cost_m} €/fm)`
    );
  }
  ...
  if (data.safetyWarnings && data.safetyWarnings.length > 0) {
    ...
    data.safetyWarnings.forEach((warn) => {
      parts.push(
        `  [${warn.severity.toUpperCase()}] ${warn.message} - ${warn.suggestion}`
      );
    });
  }
