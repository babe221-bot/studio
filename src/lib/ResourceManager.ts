...
class LRUResourceCache<T extends { dispose?: () => void }> {
  ...
  get(key: string): T | undefined {
    ...
    if (entry) {
      ...
      if (this.debug) {
        console.log(
          `[ResourceManager] Cache hit: ${this.type}:${key} (refs: ${entry.refCount})`
        );
      }
      return entry.value;
    }
  ...
  set(key: string, value: T, initialRefCount = 1): void {
    ...
    if (existing) {
      ...
      if (this.debug) {
        console.log(
          `[ResourceManager] Incremented refs for ${this.type}:${key} (refs: ${existing.refCount})`
        );
      }
      return;
    }
    ...
    if (this.debug) {
      console.log(
        `[ResourceManager] Added to cache: ${this.type}:${key} (refs: ${initialRefCount})`
      );
    }
  }
  ...
  acquire(key: string): boolean {
    ...
    if (entry) {
      ...
      if (this.debug) {
        console.log(
          `[ResourceManager] Acquired ${this.type}:${key} (refs: ${entry.refCount})`
        );
      }
      return true;
    }
  ...
  release(key: string): boolean {
    ...
    if (!entry) {
      if (this.debug) {
        console.warn(
          `[ResourceManager] Attempted to release unknown ${this.type}:${key}`
        );
      }
      return false;
    }
    ...
    if (this.debug) {
      console.log(
        `[ResourceManager] Released ${this.type}:${key} (refs: ${entry.refCount})`
      );
    }
  ...
  disposeAll(): void {
    if (this.debug) {
      console.log(
        `[ResourceManager] Disposing all ${this.type} resources (${this.cache.size} items)`
      );
    }
  ...
  private evictLRU(): void {
    ...
    if (oldestKey && oldestEntry) {
      if (this.debug) {
        console.log(
          `[ResourceManager] LRU eviction: ${this.type}:${oldestKey}`
        );
      }
  ...
  private disposeEntry(entry: CacheEntry<T>): void {
    if (entry.value.dispose) {
      try {
        ...
        console.error(
          `[ResourceManager] Error disposing ${this.type}:${entry.key}`,
          err
        );
      }
    }
...
export class ResourceManager {
  ...
  private constructor(config: Partial<ResourceCacheConfig> = {}) {
    ...
    this.textureCache = new LRUResourceCache(
      'texture',
      this.config.maxTextureCacheSize,
      this.config.debug
    );
    this.materialCache = new LRUResourceCache(
      'material',
      this.config.maxMaterialCacheSize,
      this.config.debug
    );
    this.geometryCache = new LRUResourceCache(
      'geometry',
      this.config.maxGeometryCacheSize,
      this.config.debug
    );
  ...
  public static getInstance(
    config?: Partial<ResourceCacheConfig>
  ): ResourceManager {
  ...
  getPBRMaterial(
    ...
    const material = new THREE.MeshPhysicalMaterial({
      ...
      normalScale: params.normalScale
        ? new THREE.Vector2(...params.normalScale)
  ...
  getStats(): ResourceStats {
    ...
    return {
      ...
      totalRefCount:
        textureStats.totalRefs +
        materialStats.totalRefs +
        geometryStats.totalRefs +
        otherStats.totalRefs,
    };
  }
