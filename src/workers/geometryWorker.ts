...
self.onmessage = (e: MessageEvent) => {
  try {
    ...
    console.debug('[GeometryWorker] Received job with dimensions:', {
      L,
      W,
      H,
    ...
    if (validationErrors.length > 0) {
      console.warn('[GeometryWorker] Validation warnings:', validationErrors);
      // Continue processing but log the issues - worker will handle gracefully
    } else {
      console.debug('[GeometryWorker] Validation passed');
    }
    ...
    if (aspectRatio > 4) {
      console.warn(
        '[GeometryWorker] Warning: Extreme aspect ratio detected:',
        aspectRatio.toFixed(2)
      );
    }

    console.debug('[GeometryWorker] Generating mesh with dimensions:', {
      L,
      W,
      H,
    });
    ...
    const pillars: { [key: string]: PillarSet } = {
      bl: [],
      br: [],
      fr: [],
      fl: [],
    ...
    if (hasOkapnik) {
      ...
      const oOff = 0.02; // inset from edge (20mm)
      ...
      const addGroove = (
        ...
        axisEnd: number,
        edgePos: number,
        inward: number, // edgePos = edge coord, inward = direction toward center
        axis: 'x' | 'z'
      ) => {
        ...
        const mkV = (along: number, perp: number, y: number) =>
          axis === 'x' ? addVertex(along, y, perp) : addVertex(perp, y, along);
    ...
    console.debug('[GeometryWorker] Generated mesh:', {
    ...
    if (vertices.length === 0) {
      console.error('[GeometryWorker] ERROR: No vertices generated');
    }
    if (indices.length === 0) {
      console.error('[GeometryWorker] ERROR: No indices generated');
    }
    if (indices.length % 3 !== 0) {
      console.error(
        '[GeometryWorker] ERROR: Index count not divisible by 3 (incomplete triangles)'
      );
    }
    ...
    // Rethrow or signal failure if necessary
    throw error;
  }
};
