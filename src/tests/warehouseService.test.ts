...
describe('getMaterials()', () => {
  const sampleMaterials = [
    ...
    },
    {
      ...
      density: 2.65,
      cost_sqm: 180,
      texture: 'https://example.com/kirm.png',
      color: '#E1DCC5',
    },
  ];
  ...
  it('returns an array of Material objects on success', async () => {
    ...
    expect(mockSelect).toHaveBeenCalledWith(
      'id, name, density, cost_sqm, texture, color'
    );
...
describe('getSurfaceFinishes()', () => {
  ...
  it('throws when Supabase returns an error', async () => {
    ...
    await expect(getSurfaceFinishes()).rejects.toThrow(
      'Failed to fetch surface finishes.'
    );
  });
});
...
describe('getEdgeProfiles()', () => {
  ...
  it('throws when Supabase returns an error', async () => {
    ...
    await expect(getEdgeProfiles()).rejects.toThrow(
      'Failed to fetch edge profiles.'
    );
  });
});
