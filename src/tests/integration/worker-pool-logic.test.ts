 ...
 */
function validateGeometryInput(input: GeometryJobInput): {
  errors: string[];
  warnings: string[];
} {
  const errors: string[] = [];
  ...
  // Aspect ratio check
  const aspectRatio =
    Math.max(input.L, input.W) / Math.min(input.L || 1, input.W || 1);
  if (aspectRatio > 4) {
 ...
 */
function validateMeshOutput(output: GeometryJobOutput): {
  errors: string[];
  warnings: string[];
} {
  const errors: string[] = [];
    ...
    if (output.positions && output.positions.length > 0) {
      ...
      for (let i = 0; i < output.indices.length; i++) {
        if (output.indices[i] > maxVertex) {
          errors.push(
            `Index ${output.indices[i]} at position ${i} exceeds vertex count ${maxVertex}`
          );
          break;
        }
      }
  ...
  if (!output.uvs || output.uvs.length === 0) {
    ...
    output.positions &&
    output.uvs.length / 2 !== output.positions.length / 3
  ) {
...
describe('Geometry Input Validation', () => {
  ...
  it('should warn on very large dimensions', () => {
    ...
    const result = validateGeometryInput(input);
    expect(result.warnings.some((w) => w.includes('large dimension'))).toBe(
      true
    );
  });
...
describe('Mesh Output Validation', () => {
  ...
  it('should detect incomplete triangles', () => {
    ...
    expect(result.errors).toContain(
      'Index count not divisible by 3 (incomplete triangles)'
    );
  });
  ...
  it('should detect out-of-bounds indices', () => {
    ...
    const result = validateMeshOutput(output);
    expect(result.errors.some((e) => e.includes('exceeds vertex count'))).toBe(
      true
    );
  });
