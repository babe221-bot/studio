...
function createMockProcessedEdges(
  overrides?: Partial<ProcessedEdges>
): ProcessedEdges {
  return {
    front: false,
    back: false,
  ...
  items.forEach((item, index) => {
    ...
    if (item.dims) {
      ...
      if (item.dims.length > 500 || item.dims.width > 500) {
        warnings.push(
          `${itemPrefix} Very large dimensions may not fit on PDF page`
        );
      }

      // Aspect ratio warning
      const aspectRatio =
        Math.max(item.dims.length, item.dims.width) /
        Math.min(item.dims.length || 1, item.dims.width || 1);
      if (aspectRatio > 4) {
        warnings.push(
          `${itemPrefix} Extreme aspect ratio (${aspectRatio.toFixed(2)}:1) may affect drawing`
        );
      }
    } else {
...
function validatePdfOptions(options: {
  ...
  if (!options.customerName) {
    warnings.push(
      'No customer name provided - PDF will show as anonymous/guest'
    );
  }
...
function estimatePdfSize(
  ...
  if (estimatedSizeKB > 1000) {
    warnings.push(
      `Estimated PDF size is ${(estimatedSizeKB / 1024).toFixed(1)}MB - may take longer to generate`
    );
  }
  if (estimatedPages > 10) {
    warnings.push(
      `Estimated ${estimatedPages} pages - consider splitting into multiple PDFs`
    );
  }
...
describe('PDF Generation - Order Item Validation', () => {
  ...
  it('should detect missing material name', () => {
    const items = [
      createMockOrderItem({ material: createMockMaterial({ name: '' }) }),
    ];
    const result = validateOrderItems(items);

    expect(result.isValid).toBe(false);
    expect(result.errors.some((e) => e.includes('Missing material name'))).toBe(
      true
    );
  });
  ...
  it('should detect invalid quantity', () => {
    ...
    expect(result.isValid).toBe(false);
    expect(result.errors.some((e) => e.includes('Invalid quantity'))).toBe(
      true
    );
  });
  ...
  it('should detect invalid order unit', () => {
    ...
    expect(result.isValid).toBe(false);
    expect(result.errors.some((e) => e.includes('Invalid order unit'))).toBe(
      true
    );
  });
  ...
  it('should warn when no edges are processed', () => {
    ...
    expect(result.warnings.some((w) => w.includes('No edges processed'))).toBe(
      true
    );
  });
});
...
describe('PDF Generation - Options Validation', () => {
  ...
  it('should warn on very long company name', () => {
    ...
    expect(
      result.warnings.some((w) => w.includes('Company name is very long'))
    ).toBe(true);
  });
  ...
  it('should warn on very long order number', () => {
    ...
    expect(
      result.warnings.some((w) => w.includes('Order number is very long'))
    ).toBe(true);
  });
  ...
  it('should warn on very long notes', () => {
    ...
    expect(result.warnings.some((w) => w.includes('Notes are very long'))).toBe(
      true
    );
  });
});
...
describe('PDF Generation - Size Estimation', () => {
  ...
  it('should estimate size for multiple items', () => {
    const items = Array(5)
      .fill(null)
      .map((_, i) => createMockOrderItem({ id: `ST-00${i + 1}` }));
    const result = estimatePdfSize(items, {});
